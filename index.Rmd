...
ggplot() +
  aes(x, y) +
  geom_ribbon(aes(ymin = 0, ymax = y), fill = "#7570b3", alpha = 0.25) +
  geom_segment(aes(x = 0, y = 0, xend = 0, yend = max(y)), colour = "#d95f02", size = 3) +
  annotate("text", x = 0, y = -0.025, label = "0", size = 25/.pt) +
  theme_void() +
  annotate("text", x = -2, y = 0.4, label = "high probability", size = 25/.pt) +
  annotate("text", x = 2, y = 0.1, label = "low probability", size = 25/.pt)
```
